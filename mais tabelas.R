...
plotgx <- plot_coefs(modelo1x, modelo2x,modelo3x, modelo4x, modelo5x, 
                         ...
                         inner_ci_level = .9,
                         point.shape = FALSE,
                     rescale.distributions=TRUE, plot.distributions = FALSE)
plotgx
...
plotg <- plot_coefs(modelo1, modelo2,modelo3, modelo4, modelo5, 
                     ...
                     inner_ci_level = .9,
                     point.shape = FALSE,
                     rescale.distributions=TRUE,dpar=NULL)
plotg
...
plot_models(
  ...
  m.labels = c("Fundamentalismo 1991", "Fundamentalismo 1997", "Fundamentalismo 2006"),
  show.values = FALSE, show.p = FALSE, p.shape = TRUE, std.est=TRUE 
)

plot_models(
  modelo1, modelo2, modelo3,
  m.labels = c("Fundamentalismo 1991", "Fundamentalismo 1997", "Fundamentalismo 2006"),
  show.values = TRUE, show.p = TRUE, p.shape = TRUE, std.est=TRUE 
)

myplot <- plot_models(
  modelo1, modelo2, modelo3,modelo4,modelo5,dot.size=3,legend.title = "Onda", 
  line.size=1, vline.color = "black",
  axis.labels = c(
    "Mulher", "Idade",
    "Educação", "Status Empregatício",
    "Escala de Renda","Tamanho da cidade",
    "Branco","Pós-Materialism Index","Interesse política"
  ),
  m.labels = c("1991", "1997", "2006","2014",
               "2018"),
  show.values = FALSE, show.p = TRUE, p.shape = TRUE, std.est=TRUE
)

plot(myplot,axis.lim = c(-.6: .6))

myplot
myplot$layers[[2]]$aes_params$vjust<-(-.4)
myplot$layers[[2]]$aes_params$hjust<-(.4)
myplot$layers[[1]]$aes_params$size <-(1)
myplot$layers[[3]]$aes_params$vjust<-(-.4)
myplot$layers[[3]]$aes_params$hjust<-(.4)


myplot

coefplot(modelo1, interactive=TRUE)

plot_models(modelo1, modelo2, modelo3, modelo4, modelo5,
           legend.title = "Onda",axis.lim = NULL, 
            axis.labels = c("Mulher", "Idade",
                            "Educação", "Status Empregatício",
                            "Escala de Renda","Tamanho da cidade",
                            "Branco","Pós-Materialism Index","Interesse política"),
            m.labels = c("1991", "1997", "2006","2014",
...
plot_models(modelo1, modelo2, modelo3, modelo4, modelo5,
            legend.title = "Onda",axis.lim = c(-0.1,1), 
                      m.labels = c("1991", "1997", "2006","2014",
                         "2018"),show.values = FALSE, 
            show.p = T, p.shape = TRUE, digits=4, std.est=TRUE, 
            p.threshold = c(0.05, 0.01, 0.001), 
...
# mais modelos e gráficos em MoreModelsAndGraphics.R
