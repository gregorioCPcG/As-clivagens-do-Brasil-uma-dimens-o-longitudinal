...
effects <- slopes(model1, variables = "anof", conf_level = 0.95)

effects_df <- effects %>%
  mutate(wave = factor(anof, levels = c("1991", "1997", "2006", "2014", "2018")))

figure1 <- plot_predictions(model1, condition ="anof", conf_level=.95)+
  labs(y="Fundamentalism (Predictions)",x="Wave")+
  theme_bw()+
  scale_x_discrete(labels = c("1991","1997","2006","2014","2018"))
...
figure2 <- combined_df%>%
  ...
  facet_wrap(~ anof, labeller = as_labeller(c("df1991" = "1991", "df1997" = "1997", "df2006" = "2006", "df2014" = "2014", "df2018" = "2018"))) +
  theme_tq() +
  labs(title = "",
       subtitle = "",
       x = "",
...
plot1 <- grid.arrange(figure1, figure2, ncol = 2)

library(svglite)

ggsave("plot.svg", plot = plot1, device = "svg", width = 12, height = 6, units = "in")
...
mod91<-broom::tidy(modelo1991)
mod91 %>% glimpse()

mod91 <- mod91 %>% mutate(ano = 1991)


mod97<-broom::tidy(modelo1997)
mod97 %>% glimpse()

mod97 <- mod97 %>% mutate(ano = 1997)


mod06<-broom::tidy(modelo2006)
mod06 %>% glimpse()

mod06 <- mod06 %>% mutate(ano = 2006)


mod14<-broom::tidy(modelo2014)
mod14 %>% glimpse()

mod14 <- mod14 %>% mutate(ano = 2014)

mod18<-broom::tidy(modelo2018)
mod18 %>% glimpse()

mod18 <- mod18 %>% mutate(ano = 2018)

modfinal <- rbind(mod91,mod97,mod06,mod14,mod18)
modfinal %>% glimpse()
...
modfinal <- modfinal  %>%   mutate(upper = estimate + (1.96 * std.error),
  ...
  mutate(signif = ifelse(p.value <= 0.05, "Significant (<= 0.05)", "Non-significant (> 0.05)"))
...
novos_rotulos <- c(
  ...
  "V17" = "High education",
  "V14" = "Right-wing",
  "V18" = "Religious person",
  "V16" = "Postmaterialist",
  "V15" = "PTist"
)

plot2 <- modfinal %>% ggplot(aes(x = factor(term, levels = names(novos_rotulos)), y = estimate, ymax = upper,
...
ggsave("plot2.svg", plot = plot2, device = "svg", width = 8, height = 6, units = "in")
